...
SQL_QUERIES = QueryDict({
    ...
    'build_generic_topic_embedding':
    ('SELECT customer_topic.topic,\n'
     + ',\n'.join([f'AVG(dim_{i}) AS dim_{i}' for i in range(100)])
     + '\nFROM `${gcp_project}.ML_LAB.W2V_CUSTOMER_EMBEDDINGS` customer_emb'
     + '\nINNER JOIN `${gcp_project}.SEMANTIC_BASKET_SEGMENTATION.SEMANTIC_CUSTOMER_TOPIC` customer_topic'  # noqa: E501
     + '\nUSING (customer_key)'
     + "\nWHERE customer_emb.date = DATE_TRUNC(DATE '${start_date}', MONTH) AND customer_topic.fecha_carga = DATE_TRUNC(DATE '${start_date}', MONTH) AND customer_emb.store_banner = '${store_banner}'"  # noqa: E501
     + '\nGROUP BY topic'
    ),
...
def main() -> None:  # noqa: D103
    ...
    # ---------------------------------------------------------------------
    # Get the embedded topic embeddings
    # ---------------------------------------------------------------------

    logging.info('Get the embedded topic embeddings')
    ...
    distances = pd.DataFrame(
        data=cosine_distances(
            topic_embeddings[[f'dim_{i}' for i in range(100)]],
            sku_emb[[f'dim_{i}' for i in range(100)]]
        ),
        index=topic_embeddings['topic'],
    ...
    # [ topic | material | cosine distance ]
    ...
    # for every customer and category...
        ...
        ['topic', 'category'], sort=False,
    # get only the top sku_per_category elements...
    ).head(
        sku_per_category
    # in this columns:
    )[
        ['topic', 'material', 'cosine_distance']
    ]
    ...
    distances = distances[
        ['topic', 'ean', 'relevance', 'campaign_name']
    ...
    deleteFromTable(
    table_ref = f'{gcp_project}.PERSONALIZED_CATALOG.TOPIC_DEFAULT_CATALOG_{upper_store_banner}',
