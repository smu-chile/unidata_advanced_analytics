...
SQL_QUERIES = QueryDict({
    ...
    from ( --A1
        select
            ...
            rank() over (partition by customer_key,basket_id,transaction_date,a.material order by sin_geo.nombre_promocion) as rnk
    ...
    from ( --A
        ...
        --solo Categorias que son parte del APOTEOSICO a analizar
        inner join (
            SELECT categoria,desc_categoria
            FROM `${gcp_project_2}.${schema_2}.VW_FACT_WORKFLOW`
            WHERE
                registro_valido = 'X'
                and organizacion_ventas = '1000'
                and canal_distribucion = '10'
                and (
                    SUBSTR(cast(n_promocion as STRING),1,3) in ('565','553','551')
                    or nombre_promocion='OFERTA SEMANA 28 CARNES 13,07 A 19.07'
                    or nombre_promocion like '%LOS FRESQUITOS%'
                    or nombre_promocion like '%FRES DE LA SEM%'
                    or nombre_promocion like '%FRES. DE LA S%'
                ) --CAMBIAR FECHAS
                --and cast(fecha_fin_de_promocion as date)>=cast('${min_date}' as date) --MES ANALISIS PROMOCIONES
            GROUP BY 1,2
        ) Cat
        on Cat.categoria = PRODUCT_H.category_code
    ...
    PROMO_LOOKUP AS (
        SELECT
            ...
            LPAD(CAST(FORMATO AS STRING), 2, '0') AS FORMATO
        FROM `${gcp_project_1}.${schema_1}.DW_VW_FACT_GP_PROMOTIONS`
        WHERE CAST(FECHA_CARGA AS DATE) = '${fecha_carga}'
        AND FORMATO = ${banner_nro}
    )
    select
        Mes_promocion,
    ...
    from ( --A1
        select
            ...
            APO,
            case
                when a.nombre_promocion_ppal is not null then a.nombre_promocion
                when sin_geo.nombre_promocion_ppal is not null
                    and producto_pesable = 'Si' then sin_geo.nombre_promocion
            end as nombre_promocion,
            case
                when a.nombre_promocion_ppal is not null then a.descripcion_mecanica
                when sin_geo.nombre_promocion_ppal is not null
                    and producto_pesable = 'Si' then sin_geo.descripcion_mecanica
            end as descripcion_mecanica,
            case
                when a.nombre_promocion_ppal is not null then a.TIPO_PROMOCION
                when sin_geo.nombre_promocion_ppal is not null
                    and producto_pesable = 'Si' then sin_geo.TIPO_PROMOCION
            end as TIPO_PROMOCION,
            case
                when a.nombre_promocion_ppal is not null then a.nombre_promocion_ppal
                when sin_geo.nombre_promocion_ppal is not null
                    and producto_pesable = 'Si' then sin_geo.nombre_promocion_ppal
            end as nombre_promocion_ppal,
            case
                when a.nombre_promocion_ppal is not null then a.Mes_promocion
                when sin_geo.nombre_promocion_ppal is not null
                    and producto_pesable = 'Si' then sin_geo.Mes_promocion
            end as Mes_promocion,
            producto_pesable,
            sin_geo.material as material_sin_geo,
            rank() over (partition by customer_key,basket_id,transaction_date,a.material order by sin_geo.nombre_promocion) as rnk
