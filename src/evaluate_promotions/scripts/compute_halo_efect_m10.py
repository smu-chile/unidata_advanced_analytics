...
SQL_QUERIES = QueryDict({
    ...
    Select
        ...
        ,nombre_promocion_APOTEOSICO_M10
        ,nombre_promocion_ppal_APOTEOSICO_M10
    ...
    from ( --A
        Select
            ...
            ,max(CASE WHEN TIPO_PROMOCION='APOTEOSICO M10' then WORKFLOW.nombre_promocion else null end) as nombre_promocion_APOTEOSICO_M10
            ,max(CASE WHEN TIPO_PROMOCION='APOTEOSICO M10' then WORKFLOW.nombre_promocion_ppal else null end) as nombre_promocion_ppal_APOTEOSICO_M10
            ...
            ,max(CASE WHEN TIPO_PROMOCION='APOTEOSICO M10' then 1 else 0 end) as APOTEOSICO_M10
    ...
    left join (
        SELECT
            ...
            , max(CASE WHEN p.TIPO_PROMOCION='APOTEOSICO M10' then p.nombre_promocion_ppal else null end) as nombre_promocion_ppal_EA_mat
    ...
    select
        ...
        ,APO_sin_geo
        ,M10_CICLO_sin_geo
        ,APOTEOSICO_M10_sin_geo
        ,PERECIBLES_M10_sin_geo
        ...
        ,case
            when M10_CICLO=1 then Venta_Bruta else 0
        end as Venta_Bruta_M10_CICLO
        ,case
            when APOTEOSICO_M10=1 then Venta_Bruta else 0
        end as Venta_Bruta_APOTEOSICO_M10
        ,case
            when PERECIBLES_M10=1 then Venta_Bruta else 0
        end as Venta_Bruta_PERECIBLES_M10
        ,case
            when APO=0
                and M10_CICLO=0
                and APOTEOSICO_M10=0
                and PERECIBLES_M10=0
        ...
        ,case
            when APO=0
                ...
                and APOTEOSICO_M10=0
                and PERECIBLES_M10=0
                and M10_CICLO_sin_geo=1 then Venta_Bruta
            else 0
        end as Venta_Bruta_M10_CICLO_sin_geo
        ,case
            when APO=0
                and M10_CICLO=0
                and APOTEOSICO_M10=0
                and PERECIBLES_M10=0
                and APOTEOSICO_M10_sin_geo=1 then Venta_Bruta
            else 0
        end as Venta_Bruta_APOTEOSICO_M10_sin_geo
        ,case
            when APO=0
                and M10_CICLO=0
                and APOTEOSICO_M10=0
                and PERECIBLES_M10=0
                and PERECIBLES_M10_sin_geo=1 then Venta_Bruta
            else 0
        end as Venta_Bruta_PERECIBLES_M10_sin_geo
        ,case
            when APO=0
                and TIENE_PROMO=0
                and M10_CICLO=0
                ...
                and Promo_sin_geo=1
                and APO_sin_geo=0
                and M10_CICLO_sin_geo=0
                and APOTEOSICO_M10_sin_geo=0 then Venta_Bruta
            else 0
    ...
    from (--C
        select
            ...
            ,case
                when sum(case when PERECIBLES_M10=1 then Cantidad else 0 end) over (partition by customer_key,basket_id,transaction_date)>0 then max(nombre_promocion_ppal_PERECIBLES_M10) over (partition by customer_key,basket_id,transaction_date)
                when sum(case when PERECIBLES_M10_sin_geo=1 then Cantidad else 0 end) over (partition by customer_key,basket_id,transaction_date)>0 then max(nombre_promocion_ppal_per_mat) over (partition by customer_key,basket_id,transaction_date)
                ...
                when sum(case when APOTEOSICO_M10=1 then Cantidad else 0 end) over (partition by customer_key,basket_id,transaction_date)>0 then max(nombre_promocion_ppal_APOTEOSICO_M10) over (partition by customer_key,basket_id,transaction_date)
                when sum(case when APOTEOSICO_M10_sin_geo=1 then Cantidad else 0 end) over (partition by customer_key,basket_id,transaction_date)>0 then max(nombre_promocion_ppal_EA_mat) over (partition by customer_key,basket_id,transaction_date)

                when sum(case when M10_CICLO=1 then Cantidad else 0 end) over (partition by customer_key,basket_id,transaction_date)>0 then max(nombre_promocion_ppal_M10_CICLO) over (partition by customer_key,basket_id,transaction_date)
                when sum(case when M10_CICLO_sin_geo=1 then Cantidad else 0 end) over (partition by customer_key,basket_id,transaction_date)>0 then max(nombre_promocion_ppal_cat_mat) over (partition by customer_key,basket_id,transaction_date)
    ...
    select
        ...
        ,producto_pesable
        ,case
            when nombre_promocion_ppal like '%LAS 10%' then 'M10 10 DE M10'
            when nombre_promocion_ppal like '%CICLO%' then 'M10 CICLO'
            when nombre_promocion_ppal like '%ELEGIDOS%' then 'APOTEOSICO M10'
            when nombre_promocion_ppal like '%ELE%' then 'APOTEOSICO M10'
            when nombre_promocion_ppal like '%PERECIBLES%' then 'PERECIBLES M10'
            when nombre_promocion_ppal like '%PUNTA DE PRECIO%' then 'PUNTA DE PRECIO M10'
            when nombre_promocion_ppal like '%APO%' then 'APOTEOSICO M10'
        end as Tipo_Promo
        ,nombre_promocion_ppal
        ,TIENE_PROMO
        ,APO
        ...
        ,Venta_Ticket_Apo
        ,Venta_Ticket_M10_CICLO
        ,Venta_Ticket_APOTEOSICO_M10
        ,Venta_Ticket_PERECIBLES_M10
        ,Venta_Ticket_APO_SinGeo
        ,Venta_Ticket_M10_CICLO_SinGeo
        ,Venta_Ticket_APOTEOSICO_M10_SinGeo
        ,Venta_Ticket_PERECIBLES_M10_SinGeo
        ...
        ,sum(Venta_Bruta_M10_CICLO) as  Venta_Bruta_M10_CICLO
        ,sum(Venta_Bruta_M10_CICLO_sin_geo) as  Venta_Bruta_M10_CICLO_sin_geo
        ,sum(Venta_Bruta_APOTEOSICO_M10) as  Venta_Bruta_APOTEOSICO_M10
        ,sum(Venta_Bruta_APOTEOSICO_M10_sin_geo) as  Venta_Bruta_APOTEOSICO_M10_sin_geo
        ,sum(Venta_Bruta_PERECIBLES_M10) as  Venta_Bruta_PERECIBLES_M10
        ,sum(Venta_Bruta_PERECIBLES_M10_sin_geo) as  Venta_Bruta_PERECIBLES_M10_sin_geo
    ...
    from (
        ...
        ,sum(Venta_Bruta_M10_CICLO) over (partition by customer_key, transaction_date) as Venta_Ticket_M10_CICLO
        ,sum(Venta_Bruta_APOTEOSICO_M10) over (partition by customer_key, transaction_date) as Venta_Ticket_APOTEOSICO_M10
        ,sum(Venta_Bruta_PERECIBLES_M10) over (partition by customer_key, transaction_date) as Venta_Ticket_PERECIBLES_M10

        ,sum(Venta_Bruta_APO_sin_geo) over (partition by customer_key, transaction_date) as Venta_Ticket_APO_SinGeo
        ,sum(Venta_Bruta_M10_CICLO_sin_geo) over (partition by customer_key, transaction_date) as Venta_Ticket_M10_CICLO_SinGeo
        ,sum(Venta_Bruta_APOTEOSICO_M10_sin_geo) over (partition by customer_key, transaction_date) as Venta_Ticket_APOTEOSICO_M10_SinGeo
        ,sum(Venta_Bruta_PERECIBLES_M10_sin_geo) over (partition by customer_key, transaction_date) as Venta_Ticket_PERECIBLES_M10_SinGeo
    ...
    SELECT
        ...
        venta_bruta,
        venta_bruta_apo,
        venta_bruta_M10_CICLO,
        venta_bruta_M10_CICLO_sin_geo,
        venta_bruta_APOTEOSICO_M10,
        venta_bruta_APOTEOSICO_M10_sin_geo,
        venta_bruta_PERECIBLES_M10,
        venta_bruta_PERECIBLES_M10_sin_geo,
        ...
        clientes,
        CASE
            WHEN tipo_promo = 'APOTEOSICO M10' THEN
                CASE
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.1 THEN 0
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.2 THEN 10
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.3 THEN 20
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.4 THEN 30
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.5 THEN 40
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.6 THEN 50
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.7 THEN 60
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.8 THEN 70
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 0.9 THEN 80
                    WHEN (venta_ticket_APOTEOSICO_M10 + venta_ticket_APOTEOSICO_M10_singeo) / Venta_Ticket_Total < 1.0 THEN 90
                    ELSE 100 -- Default value for percentages greater than 100%
                END
            WHEN tipo_promo = 'PERECIBLES M10' THEN
                CASE
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.1 THEN 0
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.2 THEN 10
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.3 THEN 20
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.4 THEN 30
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.5 THEN 40
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.6 THEN 50
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.7 THEN 60
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.8 THEN 70
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 0.9 THEN 80
                    WHEN (venta_ticket_PERECIBLES_M10 + venta_ticket_PERECIBLES_M10_singeo) / Venta_Ticket_Total < 1.0 THEN 90
                    ELSE 100 -- Default value for percentages greater than 100%
                END
            WHEN tipo_promo = 'M10 CICLO' THEN
                CASE
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.1 THEN 0
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.2 THEN 10
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.3 THEN 20
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.4 THEN 30
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.5 THEN 40
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.6 THEN 50
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.7 THEN 60
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.8 THEN 70
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.9 THEN 80
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 1.0 THEN 90
                    ELSE 100 -- Default value for percentages greater than 100%
            END
            WHEN tipo_promo = 'PUNTA DE PRECIO M10' THEN
                CASE
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.1 THEN 0
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.2 THEN 10
                    WHEN (venta_ticket_M10_CICLO + venta_ticket_M10_CICLO_singeo) / Venta_Ticket_Total < 0.3 THEN 20
                    ...
                    ELSE 100 -- Default value for percentages greater than 100%
            END
            WHEN tipo_promo = 'M10 10 DE M10' THEN
                CASE
...
def main():
    ...
    df_get_step_3_data = df_get_step_3_data[(df_get_step_3_data['ecommerce'] == 0) &
