...
dag_args = {
    'dag_id': 'semantic_basket_segmentation_2026',
...
with DAG(**dag_args) as dag:
    ...
    semantic_basket_topic = ExtendedDataprocCreateBatchOperator(
        task_id = 'basket_topic_predict_2026',
        python_script_path=(
            f'{PROJECT_NAME}/'
            'scripts/'
            'basket_topic_predict_2026.py'
        ),
    ...
    semantic_customer_topic = ExtendedDataprocCreateBatchOperator(
        task_id = 'customer_topic_predict_2026',
        python_script_path=(
            f'{PROJECT_NAME}/'
            'scripts/'
            'customer_topic_predict_2026.py'
        ),
        dag_env_config=dag_env_config,
        docker_image_name=f'{PROJECT_NAME}',
        pyspark_batch_args=[
