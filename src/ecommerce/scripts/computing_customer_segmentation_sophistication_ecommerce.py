...
category_counts AS (
  ...
  (SELECT
    *
  FROM
  `${proyecto}.CDA_VISTAS.VW_SALES_ITEM` A
  WHERE
  ...
  INNER JOIN `cl-cda-prod.DS_CDA_VW_SMU.DW_VW_FACT_MARKET_BASKET_E_COMMERCE` 
  FBE ON FBE.MARKET_BASKET_KEY = A.MARKET_BASKET_KEY AND FBE.CANAL_VENTA = 'E-COMMERCE'
...
-- MODIFICACION (PREGUNTARLE A ANDRES SI ERA MAS CONVENIENTE QUE EL IN AL UBER, RAPI) --

INNER JOIN `cl-cda-prod.DS_CDA_VW_SMU.DW_VW_FACT_MARKET_BASKET_E_COMMERCE`  FBE ON FBE.MARKET_BASKET_KEY = A.MARKET_BASKET_KEY 
AND FBE.CANAL_VENTA = 'E-COMMERCE'
