"""Defines the DAG that allocates personalized products."""
# Default
import json
import platform
import importlib
...
with DAG(**dag_args) as dag:
    EXECUTION_DATE = "{{ dag_run.conf.get('execution_date', dag.timezone.convert(data_interval_end).add(days=1).strftime('%Y-%m-%d')) }}"  # noqa: E501
    ...
    for store_banner in store_banner_list:
        ...
        personalized_products_task = ExtendedDataprocCreateBatchOperator(
            ...
            python_script_path=(
                f'{PROJECT_NAME}/'
                'scripts/'
                'personalized_products_sin_usuals_max_ean_sub_cat_1_3.py'
            ),
