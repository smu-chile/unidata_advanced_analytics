...
ppto_year =  str(dt.datetime.now(dt.UTC).year)
# -------------------------------------------------------------------------
# Cleaning Func
# -------------------------------------------------------------------------
def cleaning_func(file: str,df_file: pd.DataFrame) -> pd.DataFrame :
    ...
    if file.startswith('admg'):
        ...
        df_file['SELLOUT'] = (
            ...
            .str.strip()
            .str.replace(',', '.', regex=False)   # coma es el separador decimal
        )
...
def main() -> None:  # noqa: D103
    ...
    input_files = {
            'ppto' :f'{site}/Unimarc/Fuentes datos manuales/Presupuesto/{ppto_year}/PPTO FORMATOS SELLOUT RECUPERO {ppto_year}.xlsx',  # noqa: E501
