...
# Cleaning Func
# -------------------------------------------------------------------------
def cleaning_func(df, year,file):  # noqa: ANN001, ANN201, D103
    print('Before cleaning:', df)  # noqa: T201
    ...
    df['year'] = pd.to_datetime(year,format='%Y')  # noqa: DTZ007, RUF100
    df = df.replace(' nan', 0.0)  # noqa: PD901
    df = df.replace('nan', 0.0)  # noqa: PD901
    df['item_code'] = df['item_code'].astype('Float64').astype('Int64')  # noqa: E501, RUF100

    print('After cleaning:', df)  # noqa: T201
    return df
