...
def main() -> None:  # noqa: D103
    ...
    print('[Aviso] Se importa Gasto total en df_detalle4 pero ya no se ocupa')
    #----------------------------------------------------------------------
    # ENDREGION
    ...
    print('Ya no se fuerzan los productos señalados por equipo de Pricing')
    ...
    uploadFrame(
        ...
        table_ddl_json_path=os.path.join('gbq_objects',
                                        'ingest_ecommerce_product_sensibility.json'),
        project=proyecto,
        gbq_client=gbq_client,
        if_exists='append'
    )
