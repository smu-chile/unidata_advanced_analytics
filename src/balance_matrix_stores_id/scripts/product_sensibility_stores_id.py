...
def main() -> None:  # noqa: D103
    ...
    store_id_sql = ','.join(f"'{s}'" for s in store_id_list) #Para query
    store_id_str = ','.join(store_id_list) #Para guardar en df
    ...
    # Usuario
    # PARCHE 1 - Usuario especial para STORES ID
    usuario = 'product_sensibility_stores_id'
    ...
    # PARCHE 2 - Nueva tabla para STORES ID
    esquema = 'PRECIO_PROMOCIONES'
    tabla = 'PRODUCT_SENSIBILITY_STORES_ID'
    ...
    # Tabla con info sofisticacion
    # PARCHE 3 - Modificación tabla sofisticacion ahora para sector_oriente
    tabla_sofisticacion = 'CUSTOMER_SEGMENTATION_SOPHISTICATION_STORES_ID'
    ...
    # PARCHE 4 - Modificación .json para sector_oriente
    uploadFrame(
        df_final,
        table_ddl_json_path=os.path.join('gbq_objects',
                                        'ingest_product_sensibility_stores_id.json'),
        project=proyecto,
        gbq_client=gbq_client,
        if_exists='append'
    )
