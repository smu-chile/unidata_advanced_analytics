...
def main() -> None:  # noqa: D103
    ...
    esquema = 'TMP'
    #PARCHE TEMPORAL
    tabla = f'TMP_REGRESSION_DATA_{use}_aux_{store_banner_tabla}_ECOMMERCE'
    tmp_path_table_aux = f'{proyecto}.{esquema}.{tabla}'

    # Tabla temporal final
    esquema = 'TMP'
    # PARCHE TEMPORAL
    tabla = f'TMP_REGRESSION_DATA_{use}_ECOMMERCE'
    tmp_path_table = f'{proyecto}.{esquema}.{tabla}'


    # Nombre archivo Json
    if use == 'FORECAST':
        nombre_json = 'ingest_regression_processed_data_forecast_ecommerce.json'
    elif use == 'ELASTICITY':
        nombre_json = 'ingest_regression_processed_data_elasticity_ecommerce.json'
    else:
