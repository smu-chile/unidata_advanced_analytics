...
parser.add_argument(
    '--region',
    type=str,
    help='Nombre de la region (ej: "XIII Region Metropolitana")'
)
...
def main() -> None:  # noqa: D103
    ...
    store_id_list = sorted(df_tiendas_region['STORE_ID'].astype(str).tolist(), key=int)

    if not store_id_list:
        logging.warning(
            f"No se encontraron tiendas para la region '{region}' -- "
            f"posiblemente el banner '{store_banner}' no tiene presencia "
            "en esta region. Se termina la corrida sin subir nada."
        )
        return
    ...
    logging.info(f'Tiendas resueltas para la region "{region}": {len(store_id_list)}')
    ...
    esquema = 'TMP'
    tabla = f'TMP_REGRESSION_DATA_{use}_aux_{store_banner_tabla}_REGION'
    tmp_path_table_aux = f'{proyecto}.{esquema}.{tabla}'

    # Tabla temporal final
    # Parche 2: ajuste nombre tabla final.
    esquema = 'TMP'
    tabla = f'TMP_REGRESSION_DATA_{use}_REGION'
    ...
    if use == 'ELASTICITY':
        nombre_json = 'ingest_regression_processed_data_elasticity_region.json'
    else:
    ...
    # Chequeo real de "sin presencia": la region puede tener tiendas de
    ...
    if n_filas_aux == 0:
        logging.warning(
            f"El banner '{store_banner}' no tiene ventas en la region "
            f"'{region}' dentro de la ventana consultada -- se termina "
