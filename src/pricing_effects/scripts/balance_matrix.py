...
WITH tabla_fecha_max AS (
  ...
  FROM `${proyecto}.PRECIO_PROMOCIONES.TMP_REGRESSION_PROCESSED_DATA_ELASTICITY`
  WHERE STORE_BANNER = '${store_banner}'
)
...
def main() -> None:
    ...
    df_balance_matrix_sp = df_balance_matrix_sp.drop(columns=['material_padre'], errors='ignore')
    ...
    df_balance_matrix_sp = df_balance_matrix_sp.drop(columns=['kvi', 'segmento_bm'], errors='ignore')  # noqa: E501
