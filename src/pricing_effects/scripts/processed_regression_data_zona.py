# Default
"""Tablon de regresion por zona -- Unimarc.

Arma el tablon de datos de regresion (transacciones, sustitutos,
apoteosicos, etc.) filtrado a una zona comercial especifica (via
TMP_TIENDAS_ACTIVAS_POR_ZONA), analogo a
processed_regression_data_region.py pero segmentado por zona en vez
de region geografica.
"""
...
parser.add_argument(
    '--zona',
    type=str,
    help='Nombre de la zona (ej: "Premium")'
)
...
'query_tiendas_por_zona':
"""
SELECT DISTINCT store_id AS STORE_ID
FROM `${proyecto}.PRECIO_PROMOCIONES.TMP_TIENDAS_ACTIVAS_POR_ZONA`
...
FROM (
  ...
  WHERE TRANSACTION_DATE >= DATE('${fecha_inicial}')
    AND TRANSACTION_DATE <= DATE('${fecha_final}')
) A
...
def main() -> None:  # noqa: D103
    ...
    # Resolver que tiendas pertenecen a esta zona -- el resto del
    # pipeline queda igual, solo cambia el listado de tiendas de entrada
    query_tiendas_por_zona = SQL_QUERIES['query_tiendas_por_zona'].substitute(
        zona=zona,
        store_banner=store_banner,
        proyecto=proyecto,
    )
    df_tiendas_zona = readBigQuery(
        query=query_tiendas_por_zona,
        user=usuario,
        gbq_client=gbq_client,
    )
    store_id_list = sorted(df_tiendas_zona['STORE_ID'].astype(str).tolist(), key=int)

    if not store_id_list:
        logging.warning(
            f"No se encontraron tiendas para la zona '{zona}' -- "
            f"posiblemente el banner '{store_banner}' no tiene presencia "
            "en esta zona. Se termina la corrida sin subir nada."
        )
        return
    ...
    logging.info(f'Tiendas resueltas para la zona "{zona}": {len(store_id_list)}')
    ...
    esquema = 'TMP'
    tabla = f'TMP_REGRESSION_DATA_{use}_aux_{store_banner_tabla}_ZONA'
    tmp_path_table_aux = f'{proyecto}.{esquema}.{tabla}'

    # Tabla temporal final
    # Parche 2: ajuste nombre tabla final.
    esquema = 'TMP'
    tabla = f'TMP_REGRESSION_DATA_{use}_ZONA'
    ...
    if use == 'ELASTICITY':
        nombre_json = 'ingest_regression_processed_data_elasticity_zona.json'
    else:
    ...
    # Chequeo real de "sin presencia": la zona puede tener tiendas de
    # OTROS banners, pero ninguna del banner actual -- recien se sabe
    # con certeza aca, despues del filtro por STORE_BANNER de la query.
    n_filas_aux = readBigQuery(
        query=f'SELECT COUNT(*) AS n FROM `{tmp_path_table_aux}`',  # noqa: S608
        user=usuario,
        gbq_client=gbq_client,
    )['n'].iloc[0]
    ...
    if n_filas_aux == 0:
        logging.warning(
            f"El banner '{store_banner}' no tiene ventas en la zona "
            f"'{zona}' dentro de la ventana consultada -- se termina "
            "la corrida sin subir nada."
        )
        return

    logging.info(f'Filas en la tabla auxiliar: {n_filas_aux:,}')
    ...
    columnas_schema_orden = [
        'STORE_BANNER', 'ZONA', 'CATEGORY_DESCRIPTION', 'SUB_CATEGORY_DESCRIPTION',
    ...
    #Parche 4: tabla final identificada por ZONA, no por listado
    if use == 'ELASTICITY':
        deleteFromTable(table_ref='cl-bigdata-analytics-preprod.PRECIO_PROMOCIONES.TMP_REGRESSION_PROCESSED_DATA_ELASTICITY_ZONA',
        ...
        uploadFrame(
            ...
            project=proyecto,
            gbq_client=gbq_client,
            if_exists='append' #elimina-inserta. 'append' sobrescribe, si no existe crea la tabla.
        )
