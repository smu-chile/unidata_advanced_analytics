...
def main() -> None:  # noqa: D103
    ...
    # Usuario
    usuario = 'ecommerce_regression_data'

    # Tabla temporal aux
    store_banner_tabla = store_banner

    esquema = 'TMP'
    tabla = f'TMP_ECOMMERCE_REGRESSION_DATA_{use}_aux_{store_banner_tabla}'
    tmp_path_table_aux = f'{proyecto}.{esquema}.{tabla}'

    # Tabla temporal final
    esquema = 'TMP'
    tabla = f'TMP_ECOMMERCE_REGRESSION_DATA_{use}'
    tmp_path_table = f'{proyecto}.{esquema}.{tabla}'


    # Nombre archivo Json
    if use == 'ELASTICITY':
        nombre_json = 'ingest_ecommerce_regression_processed_data_elasticity.json'
    else:
    ...
    if use == 'ELASTICITY':
        deleteFromTable(
            table_ref='cl-bigdata-analytics-preprod.PRECIO_PROMOCIONES.TMP_ECOMMERCE_REGRESSION_PROCESSED_DATA_ELASTICITY',
        ...
        uploadFrame(
            df_final,
            table_ddl_json_path=os.path.join('gbq_objects',nombre_json),
            project=proyecto,
            gbq_client=gbq_client,
            if_exists='append'
        )
