...
def _entrenar_elasticidad(df_material: pd.DataFrame) -> dict:
    ...
    IMPORTANTE (especifico de esta tabla, solo-promocion): en el
    modelo combinado 'Regular' servia de categoria de referencia
    implicita (dummies=0 en esos dias). Aca TODOS los dias son de
    promocion, asi que cada fila cae en exactamente 1 bucket -- si se
    dejan todos los buckets presentes como dummies, su suma es
    siempre 1, exactamente igual a la constante (colinealidad
    perfecta, matriz de diseño con rango deficiente). Se excluye
    explicitamente el bucket de MENOR profundidad presente como
    referencia, antes de armar las variables candidatas -- el filtro
    VIF por si solo NO detecta este tipo de colinealidad (solo mira
    correlacion entre las variables candidatas, nunca incluye la
    constante en ese calculo).
    """
    vacio = {'modelo': None, 'variables': [], 'r2_ajustado': np.nan}

    buckets_presentes = [
        b for b in ORDEN_INTENSIDAD
        if b in df_material.columns and df_material[b].nunique() > 1  # noqa: PD101
    ]
    bucket_referencia = buckets_presentes[0] if buckets_presentes else None
    buckets_a_usar = [b for b in buckets_presentes if b != bucket_referencia]

    variables_candidatas = ['log_precio', *buckets_a_usar, *CANDIDATAS_MECANICA]
    ...
    return {
        ...
        'r2_ajustado': modelo.rsquared_adj, 'bucket_referencia': bucket_referencia,
    }
...
def asignar_segmento_elasticidad(elasticidades: pd.Series) -> pd.Series:
    ...
    banners/tablas con poca evidencia, ej. Ecommerce en
    ELASTICITY_PROMOCIONAL), devuelve la serie vacia (NaN) en vez de
    fallar -- no se puede clasificar en high/low lo que no existe.
    """
...
def main() -> None:  # noqa: D103
    ...
    # FILTRO CLAVE de esta tabla: solo dias de PROMOCION (cualquier
    # bucket de intensidad, nunca Regular). Los materiales que viven
    # casi siempre en Regular (con muy pocos dias de promo) quedaran
    ...
    logging.info(f'Panel filtrado a solo dias de promocion: {len(df_panel):,} de '
    ...
    for material_ean, df_material in df_panel.groupby('material_ean'):
        ...
        fila = {
            ...
            'bucket_referencia_promo': info_modelo.get('bucket_referencia'),
        }
    ...
    uploadFrame(
        ...
        table_ddl_json_path=os.path.join(
            'gbq_objects', 'ingest_elasticity_promocional.json'),
        project=proyecto,
        gbq_client=gbq_client,
        if_exists='append'
    )
