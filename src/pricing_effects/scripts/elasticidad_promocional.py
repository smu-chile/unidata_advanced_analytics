# Default
"""Elasticidad de precios V7.x -- metodo promocional (sin cascada).

Calcula elasticidad precio-cantidad por material para un store_banner,
usando SOLO periodos con estado != 'Regular' (es decir, promocionales --
sin mezclar regimenes) y SIN cascada de fallback -- unicamente se
reporta evidencia propia. El resultado se sube a
PRECIO_PROMOCIONES.ELASTICITY_PROMOCIONAL en BigQuery.
"""
...
COLUMNAS_FEATURES = [
    'frecuencia_cambio_precio', 'n_niveles_precio', 'cv_cantidad',
    'pct_dias_sin_venta', 'racha_max_sin_venta', 'salto_nivel_tercios',
    'pendiente_tendencia', 'autocorrelacion_1', 'asimetria_cantidad',
...
UMBRAL_R2_CLIP = 0.70
UMBRAL_HIGH = 0.30
N_JOBS = 2  # ver justificacion en elasticidad_general.py -- loky
            # (procesos separados) duplica memoria por worker
BATCH_SIZE = 25
BATCH_SIZE_IC = 25
N_JOBS_IC = 2
...
COLUMNAS_FINALES_ORDENADAS = [
    ...
    'EAN', 'UMV', 'CLUSTER', 'ORIGEN', 'ELASTICIDAD',
    'SEGMENTO_ELASTICIDAD', 'N_Eventos', 'METODO',
    'SCORE_CONFIABILIDAD', 'CONFIABILIDAD',
]
...
FECHAS_FERIADO = pd.to_datetime([
    ...
    '2024-09-18', '2025-09-18', '2024-09-19', '2025-09-19',
    '2024-10-31', '2025-10-31',
    '2024-12-24', '2025-12-24',
    '2024-12-31', '2025-12-31',
])
COLUMNAS_CALENDARIO = [
    'estacional_sin', 'estacional_cos', 'dias_hasta_feriado_acotado',
    'es_quincena', 'martes', 'miercoles', 'jueves', 'viernes',
    'sabado', 'domingo',
]

# ====================================================================
# VARIABLES PROMOCIONALES -- DELIBERADAMENTE VACIAS EN ESTA VARIANTE
# ====================================================================
# Esta version del pipeline filtra el panel a UN SOLO regimen (regular
# o promocional) -- meter variables de intensidad promocional como
# covariable no tendria sentido en ninguno de los 2 casos:
#   - Filtrando a Regular: la intensidad quedaria constante en cero
#     (sin variacion que estimar).
#   - Filtrando a Promocional: la intensidad es casi identica a
#     log_precio (se DEFINE por la profundidad del descuento) --
#     incluirlos juntos seria colinealidad severa, no un control real.
# Como todas las funciones de entrenamiento usan *COLUMNAS_PROMOCIONALES
# (desempacado), dejarla vacia las excluye automaticamente sin tocar
# ninguna funcion.
# ====================================================================
COLUMNAS_MECANICA_PROMOCIONAL = []
ORDEN_INTENSIDAD = []
COLUMNAS_PROMOCIONALES = []

CLUSTERS_CON_TENDENCIA = {
    'tendencia_fuerte', 'lanzamiento', 'rampa_ascendente', 'rampa_descendente',
}

# ====================================================================
# MODELOS_POR_TIPO -- variante regular/promocional
# ====================================================================
# DIFERENCIA CLAVE vs. metodo general: 'intermitente' usa SOLO Log_log,
# sin Intermittent_GAMM. Ese metodo usa log_gap, y al filtrar el panel
# a un solo regimen, los dias excluidos generan huecos que log_gap no
# puede distinguir de un quiebre de stock real -- eso podia inflar
# artificialmente la elasticidad. Log_log nunca usa log_gap, asi que
# queda inmune a ese problema. El label 'intermitente' sigue existiendo
# (el clustering lo detecta igual), pero el numero final de elasticidad
# nunca pasa por esa variable.
# ====================================================================
MODELOS_POR_TIPO = {
    'limpio': ['Log_log', 'GAM'],
    'ciclos_rapidos': ['GAMM_like', 'GAM', 'RDD'],
    'intermitente': ['Log_log'],
...
SQL_QUERIES = QueryDict({  # Region: Explicacion de query
    ...
    # Panel principal -- cantidad_total/precio_promedio CRUDOS. Sin
    # ean_sustituto_1 (esta variante no tiene cascada, ese dato nunca
    # se usaria) y sin mecanica promocional (COLUMNAS_PROMOCIONALES
    # queda vacia a proposito en esta variante).
    'query_panel':
    """
    SELECT
    ...
    FROM `${table}`
    WHERE STORE_BANNER = '${store_banner}' AND CANTIDAD_TOTAL > 0
    ORDER BY material, ean, p_date
    """,

    # estado + sales_uom (UMV) -- estado se usa para filtrar el panel
    # a un solo regimen (Regular o Promocional, ver main()).
    # Deduplicacion: keep='last'.
    'query_estado_umv':
    """
    SELECT
        MATERIAL AS material, EAN AS ean, P_DATE AS p_date,
...
def main() -> None:  # noqa: D103
    ...
    # -------------------------------------------------------------------
    usuario = 'elasticidad_promocional'
    esquema = 'PRECIO_PROMOCIONES'
    tabla = 'ELASTICITY_PROMOCIONAL'
    # Las 2 tablas de regresion (fisica y ecommerce) migraron de TMP a
    # PRECIO_PROMOCIONES. Esta variante no maneja ecommerce -- solo
    # banners fisicos.
    dataset_regression = 'PRECIO_PROMOCIONES'
    table_regression = f'{proyecto}.{dataset_regression}.TMP_REGRESSION_PROCESSED_DATA_ELASTICITY'
    ...
    query_panel = SQL_QUERIES['query_panel'].substitute(
        table=table_regression, store_banner=store_banner)
    df_panel = readBigQuery(
        query=query_panel, user=usuario, gbq_client=gbq_client)
    ...
    logger.info(
        f"Panel COMPLETO: {df_panel.shape}, "
        f"{df_panel['material_ean'].nunique():,} combinaciones"
    )

    # REGION: PASO 1B -- filtrar panel a SOLO periodos PROMOCIONALES
    # -------------------------------------------------------------------
    # Trae 'estado' desde BASELINE_PANEL (misma deduplicacion keep='last'
    # que el resto del pipeline), y filtra el panel COMPLETO a solo dias
    # con estado != 'Regular' (promocional) -- ANTES de calcular
    # calendario, features, clustering, o entrenar cualquier modelo.
    ...
    df_estado_raw['material_ean'] = df_estado_raw['material'] + '_' + df_estado_raw['ean']
    df_estado = df_estado_raw.drop_duplicates(subset=['material_ean', 'p_date'], keep='last')
    df_panel = df_panel.merge(
        df_estado[['material_ean', 'p_date', 'estado', 'umv']],
        on=['material_ean', 'p_date'], how='left',
    )
    n_antes_filtro = len(df_panel)
    df_panel = df_panel[df_panel['estado'] != 'Regular'].copy()
    logger.info(
        f'\nPanel filtrado a PROMOCIONAL (no-Regular): {len(df_panel):,} filas de '
        f'{n_antes_filtro:,} ({len(df_panel) / n_antes_filtro * 100:.1f}%)'
    )
    logger.info(f"Materiales unicos tras filtro: {df_panel['material_ean'].nunique():,}")
    # ENDREGION

    # REGION: PASO 1C -- precio_suavizado sobre el panel YA FILTRADO
    # -------------------------------------------------------------------
    # SIEMPRE se calcula, pero no decide nada por si solo -- es un
    # candidato adicional que solo se usa en el reintento posterior. Se
    # calcula DESPUES del filtro de regimen -- la mediana movil se
    # computa SOLO con los dias del regimen correspondiente.
    df_panel = df_panel.sort_values(['material_ean', 'p_date']).reset_index(drop=True)
    df_panel['precio_suavizado'] = df_panel.groupby('material_ean')['precio_promedio'].transform(
        lambda s: s.rolling(
            VENTANA_SUAVIZADO_DIAS, center=True, min_periods=VENTANA_SUAVIZADO_DIAS // 2
        ).median()
    )
    df_panel['precio_suavizado'] = df_panel.groupby('material_ean')['precio_suavizado'].transform(
        lambda s: s.bfill().ffill()
    )
    logger.info(
        'precio_suavizado calculado sobre el panel ya filtrado '
        '(mediana movil 45 dias centrada).'
    )
    # ENDREGION
    ...
    # ================================================================
    # ROSTER -- SOLO PROPIA, SIN CASCADA DE FALLBACK
    # ================================================================
    # Esta variante NO hereda de sustituto/subcategoria/categoria -- solo
    # se reporta la elasticidad de materiales que lograron pasar el
    # Quality Gate con evidencia PROPIA. Los que no calificaron quedan
    # marcados 'sin_evidencia' y se excluyen del roster final mas
    # adelante (ver filtro final, despues del shrinkage).
    #
    # 'mediana_subcat' SI se mantiene -- no como fallback de cobertura,
    # sino porque el shrinkage la usa como prior hacia el cual atenuar
    # la estimacion propia. Es el unico uso que le queda a la nocion
    # de "subcategoria" en esta variante.
    # ================================================================
    cols_categoria = [
        c
        for c in [
    ...
    df_categorias = df_panel.groupby('material_ean')[cols_categoria].first().reset_index()
    ...
    cols_ganador = [
        c
        for c in [
            'material_ean', 'metodo', 'elasticidad', 'ic_lower', 'ic_upper',
            'ic_width', 'ic_crosses_zero', 'wape', 'mae_log', 'bias', 'r2',
            'corr', 'price_response_error_10pct', 'price_response_monotonic',
            'score_predictivo', 'score_total', 'evidencia_ic', 'n_train',
        ]
        if c in df_ganadores.columns
    ]
    roster = roster.merge(df_ganadores[cols_ganador], on='material_ean', how='left')
    roster['tipo_evidencia'] = np.where(roster['elasticidad'].notna(), 'propia', 'sin_evidencia')
    roster.loc[roster['elasticidad'].isna(), 'metodo'] = 'sin_evidencia'
    roster['elasticidad_final'] = roster['elasticidad']
    # Sin niveles de herencia -- solo 2 estados: propia o
    # sin_evidencia
    roster['nivel_herencia'] = np.where(
        roster['tipo_evidencia'] == 'propia', 'propia', 'sin_evidencia_definitivo'
    )
    roster['tipo_elasticidad'] = np.where(
        roster['tipo_evidencia'] == 'propia', 'propia', 'sin_evidencia'
    )
    # mediana_subcat: prior para el shrinkage siguiente, SOLO calculada
    # con materiales que tienen evidencia propia
    ...
    n_universo_panel = df_categorias['material_ean'].nunique()
    logger.info(f'Universo total: {n_universo_panel:,} material_ean')
    n_propia = roster['tipo_evidencia'].eq('propia').sum()
    logger.info(f'Con evidencia PROPIA (pasan el Quality Gate): {n_propia:,}')
    n_sin_evidencia = roster['tipo_evidencia'].eq('sin_evidencia').sum()
    logger.info(
        f'Sin evidencia (se excluiran del roster final, sin heredar nada): '
        f'{n_sin_evidencia:,}'
    )
    ...
    # ================================================================
    # FILTRO FINAL -- SOLO MATERIALES CON EVIDENCIA PROPIA
    # ================================================================
    # A partir de aca, 'roster' queda reducido a SOLO los material_ean que
    # lograron una elasticidad propia (con shrinkage ya aplicado). Todo lo
    # que sigue (segmentacion, carga a BigQuery) opera sobre este roster
    # ya filtrado -- nada heredado, nada sin evidencia se reporta.
    # ================================================================
    n_antes_filtro_final = len(roster)
    roster = roster[roster['tipo_evidencia'] == 'propia'].copy()
    n_despues_filtro_final = len(roster)
    logger.info('=' * 80)
    logger.info('FILTRO FINAL -- SOLO EVIDENCIA PROPIA')
    logger.info('=' * 80)
    logger.info(f'Material_ean antes del filtro: {n_antes_filtro_final:,}')
    logger.info(f'Material_ean con evidencia propia (se reportan): {n_despues_filtro_final:,}')
    n_excluidos = n_antes_filtro_final - n_despues_filtro_final
    pct_excluidos = n_excluidos / n_antes_filtro_final * 100 if n_antes_filtro_final else 0.0
    logger.info(
        f'Excluidos (sin suficiente historia para elasticidad propia): '
        f'{n_excluidos:,} ({pct_excluidos:.1f}%)'
    )
    ...
    # REGION: Carga a BigQuery -- incremental por banner
    # -------------------------------------------------------------------
    ...
    uploadFrame(
        roster_slim,
        table_ddl_json_path=os.path.join('gbq_objects', 'ingest_elasticity_promocional.json'),
        project=proyecto,
        gbq_client=gbq_client,
        if_exists='append'
    )
