...
category_counts AS (
  ...
  WHERE
    ...
    AND DS.STORE_ID IN (${lista_store_ids})
...
WHERE
  ...
  AND D.STORE_ID IN (${lista_store_ids})
...
def main() -> None:  # noqa: D103
    ...
    # script filtra por esta lista de STORE_ID en vez de por
    # store_banner completo (ver processed_regression_data_zona.py,
    # mismo patron).
    query_tiendas_zona = SQL_QUERIES['query_tiendas_zona'].substitute(
        proyecto=proyecto, store_banner=store_banner, zona=zona
    )
    df_tiendas_zona = readBigQuery(
        query=query_tiendas_zona, user=usuario, gbq_client=gbq_client
    )
    lista_store_ids_zona = sorted(
        df_tiendas_zona['STORE_ID'].astype(str).tolist(), key=int
    )
    if not lista_store_ids_zona:
        logging.warning(
            f"No se encontraron tiendas para la zona '{zona}' -- se "
            "termina la corrida sin subir nada."
        )
        return
    logging.info(
    ...
    tabla = 'PRODUCT_SENSIBILITY_ZONA'
    ...
    query_principal = SQL_QUERIES['query_principal'].substitute(
            ...
            lista_store_ids=lista_store_ids,
...
# para monitoreo de cálculo de sensibilidad.
#
    ...
    uploadFrame(
        ...
        table_ddl_json_path=os.path.join('gbq_objects',
                                        'ingest_product_sensibility_zona.json'),
        project=proyecto,
        gbq_client=gbq_client,
        if_exists='append'
    )
