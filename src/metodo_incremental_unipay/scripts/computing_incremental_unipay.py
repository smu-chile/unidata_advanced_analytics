 ...
 DECLARE ANOMES_CERRADO INT64 DEFAULT ${anomes_cerrado};
    ...
    *
    ...
    -- SUBQUERY --
      ...
      WITH CLIENTE_FORMATO AS (
      --- CLIENTES FORMATO ---
          SELECT
                DISTINCT DEID.pda_customer_key AS CUSTOMER_KEY,
                ANOMES_CERRADO,
                ...
                CASE WHEN DSH.STE_ID in ('01','15','02') THEN 'Norte Grande'
                      WHEN DSH.STE_ID in ('04','03') THEN 'Norte Chico'
                      WHEN DSH.STE_ID in ('05','06','07') THEN 'Central'
                      WHEN DSH.STE_ID in ('13') THEN 'Metropolitana'
                      WHEN DSH.STE_ID in ('09','08','14','10','16') THEN 'Sur'
                      WHEN DSH.STE_ID in ('11','12') THEN 'Austral'
                      END AS ZONA,
                      CASE WHEN (A.TNDR_TP_ID IN ('35','68','78') OR B.TNDR_TP_ID IN ('35','68','78')) THEN 'Unipay'
                      WHEN (A.TNDR_TP_ID IN ('42','44','45','46','60','61','62','64','65','66','67'
          ,'69','70','79','83','84','85','86','89','90','91','93','94','95','96','97') OR B.TNDR_TP_ID IN ('42','44','45','46','60','61','62','64','65','66','67','69','70','79','83','84','85','86','89','90','91','93','94','95','96','97')) THEN 'Tarjeta Credito'
                      WHEN (A.TNDR_TP_ID IN ('11') OR B.TNDR_TP_ID IN ('11')) THEN 'Efectivo'
                      WHEN (A.TNDR_TP_ID IN ('63','82','92') OR B.TNDR_TP_ID IN ('63','82','92')) THEN 'Debito'
                      ELSE 'Resto' END AS MEDIO_DE_PAGO,
                      CASE
                        WHEN DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) < 25
                            THEN 'MENOR A 25'
                        WHEN DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) < 35
                            THEN '25 - 34'
                        WHEN DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) < 50
                            THEN '35 - 49'
                        WHEN DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) < 65
                            THEN '50 - 64'
                        ELSE '65 O MÁS'
                      END AS RANGO_EDAD,
                CASE WHEN SHA.SHABIT = 'VIP Platino' THEN 'VIP'
                    WHEN SHA.SHABIT IS NULL THEN 'BRONCE'
                    WHEN SHA.SHABIT = 'EN FUGA' THEN 'BRONCE'
                    ELSE SHA.SHABIT END AS SHABITS,
                SPH.CLASIFICACION_CLIENTE,
                SUM(FIT.ITM_TXN_AMT - FIT.TAX_AMOUNT) AS VENTA_NETA,
                COUNT(DISTINCT CASE WHEN FIT.ITM_TXN_AMT > 0 THEN FIT.CUSTOMER_KEY END) AS CLIENTES
            ...
            FROM
                  ...
                  INNER JOIN `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_CDA_CST_DEID` DEID ON DEID.CUSTOMER_KEY  = FIT.CUSTOMER_KEY
                  ...
                  LEFT JOIN
                              (
                                  SELECT DISTINCT CUSTOMER_KEY, CLASIFICACION_CLIENTE
                                FROM `cl-bigdata-analytics-preprod.CONOCIMIENTO_CLIENTE.CUSTOMER_SEGMENTATION_SOPHISTICATION`  SO
                                WHERE CAST(SO.MONTHID AS INT64) = ANOMES_CERRADO
                                and STORE_BANNER = 'Unimarc'
                                ) SPH
            ON
              SPH.CUSTOMER_KEY =  FIT.CUSTOMER_KEY
                  ...
                  (
                    SELECT *
                    FROM
                    `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_FACT_MONTH_CUSTOMER_ORGANIZATION_SHABITS_UNIDATA`
                    WHERE
                    MONTHID = ANOMES_CERRADO
                    AND ORG_IP_ID IN ('01')
                  )
            ...
            ON
              O.CUSTOMER_KEY = FIT.CUSTOMER_KEY
              AND O.ORG_IP_ID = DSH.ORG_IP_ID
              AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = CAST(FORMAT_DATE('%Y%m',DATE_ADD(PARSE_DATE('%Y%m',CAST(O.MONTH_ID AS STRING)), INTERVAL -1 MONTH)) AS INT64)
                  ...
                  -- AND FIT.CUSTOMER_HEX NOT IN ('3588d47a76aac91fcf2a3c2f55f6a351')
                    ...
                    AND DSH.ORG_IP_ID IN ('01')
                    AND FBE.MARKET_BASKET_KEY IS NULL
                    --AND COL.CUSTOMER_KEY IS NULL
                    AND O.CUSTOMER_KEY IS NULL
                    AND SPH.CUSTOMER_KEY IS NOT NULL
                    AND EQX.fecha_nacimiento IS NOT NULL
                    AND DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) >= 25
                    AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = ANOMES_CERRADO
                  GROUP BY
                    1,2,3,4,5,6,7,8
                  ORDER BY 1 ASC
      ), CLIENTES_TH_NO_USA AS (
      -- EXTRAER CLIENTES TH QUE NO USAN
          SELECT
          DISTINCT DEID.pda_customer_key AS CUSTOMER_KEY,
          ANOMES_CERRADO,
          CASE WHEN DSH.ORG_IP_ID IN ('01') AND FBE.MARKET_BASKET_KEY IS NULL THEN 'Unimarc'
                ...
                ELSE 'Sin Formato' END AS store_banner,
          CASE
            WHEN DATE_DIFF(CURRENT_DATE(), CAST(CONTAC.FECNACIMIENTO AS DATE), YEAR) < 25
                THEN 'MENOR A 25'
            WHEN DATE_DIFF(CURRENT_DATE(), CAST(CONTAC.FECNACIMIENTO AS DATE), YEAR) < 35
                THEN '25 - 34'
            WHEN DATE_DIFF(CURRENT_DATE(), CAST(CONTAC.FECNACIMIENTO AS DATE), YEAR) < 50
                THEN '35 - 49'
            WHEN DATE_DIFF(CURRENT_DATE(), CAST(CONTAC.FECNACIMIENTO AS DATE), YEAR) < 65
                THEN '50 - 64'
            ELSE '65 O MÁS'
        END AS RANGO_EDAD
            FROM
                  ...
                  INNER JOIN `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_CONTACTABILITY_UNIPAY` CONTAC ON CONTAC.CUSTOMER_KEY = FIT.CUSTOMER_KEY
                  ...
                  LEFT JOIN `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_FACT_MONTH_CUSTOMER_COLLABORATOR` COL ON COL.CUSTOMER_KEY = FIT.CUSTOMER_KEY AND
          COL.MONTH_ID = (DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER)
            ...
            ON
              ...
              AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = CAST(FORMAT_DATE('%Y%m',DATE_ADD(PARSE_DATE('%Y%m',CAST(O.MONTH_ID AS STRING)), INTERVAL -1 MONTH)) AS INT64)
                  ...
                  -- AND FIT.CUSTOMER_HEX NOT IN ('3588d47a76aac91fcf2a3c2f55f6a351')
                    ...
                    AND FBE.MARKET_BASKET_KEY IS NULL
                    AND DSH.STORE_ID NOT IN ('0622')
                    AND (A.TNDR_TP_ID NOT IN ('35','68','78') OR B.TNDR_TP_ID NOT IN ('35','68','78'))
                    AND DSH.ORG_IP_ID IN ('01')
                    AND FBE.MARKET_BASKET_KEY IS NULL
                    --AND COL.CUSTOMER_KEY IS NULL
                    AND O.CUSTOMER_KEY IS NULL
                    AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = ANOMES_CERRADO
                    AND DATE_DIFF(CURRENT_DATE(), CAST(CONTAC.FECNACIMIENTO AS DATE), YEAR) >= 25
                  GROUP BY
                    1,2,3,4
                  ORDER BY 1 ASC
      ), CLIENTES_TH_USA AS (
      -- EXTRAER TH QUE USAN --
        SELECT
            ...
            COL.MONTH_ID = (DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER)
              ...
              LEFT JOIN `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_CONTACTABILITY_UNIPAY` CONTAC ON CONTAC.CUSTOMER_KEY = FIT.CUSTOMER_KEY
      ...
      SELECT
      FORMATO.CUSTOMER_KEY,
      FORMATO.ANOMES_CERRADO AS PERIODO,
      FORMATO.STORE_BANNER,
      'PERSONA' AS TIPO_CLIENTE,
      FORMATO.ZONA,
      FORMATO.MEDIO_DE_PAGO,
      COALESCE(
              NULLIF(CLIENTES_TH_USA.RANGO_EDAD, ''),
              NULLIF(CLIENTES_TH_NO_USA.RANGO_EDAD, ''),
              NULLIF(FORMATO.RANGO_EDAD, '')
          ) AS RANGO_EDAD,
      FORMATO.SHABITS,
      FORMATO.CLASIFICACION_CLIENTE,
      FORMATO.VENTA_NETA,
      FORMATO.CLIENTES,

      CASE WHEN CLIENTES_TH_NO_USA.CUSTOMER_KEY IS NULL AND CLIENTES_TH_USA.CUSTOMER_KEY IS NULL THEN 'CLIENTE FORMATO'
          WHEN CLIENTES_TH_NO_USA.CUSTOMER_KEY IS NOT NULL AND CLIENTES_TH_USA.CUSTOMER_KEY IS NULL THEN 'CLIENTE TH NO USA'
          WHEN CLIENTES_TH_USA.CUSTOMER_KEY IS NOT NULL THEN 'CLIENTE TH USA' ELSE 'NO CLASIFICA' END AS TARGET_CLIENTE
      FROM CLIENTE_FORMATO FORMATO
      LEFT JOIN CLIENTES_TH_NO_USA ON FORMATO.CUSTOMER_KEY = CLIENTES_TH_NO_USA.CUSTOMER_KEY
      LEFT JOIN CLIENTES_TH_USA ON FORMATO.CUSTOMER_KEY = CLIENTES_TH_USA.CUSTOMER_KEY
      WHERE  COALESCE(
              NULLIF(CLIENTES_TH_USA.RANGO_EDAD, ''),
              NULLIF(CLIENTES_TH_NO_USA.RANGO_EDAD, ''),
              NULLIF(FORMATO.RANGO_EDAD, '')
    ...
    --- CLIENTES FORMATO ---
        ...
        ,'69','70','79','83','84','85','86','89','90','91','93','94','95','96','97') OR B.TNDR_TP_ID IN ('42','44','45','46','60','61','62','64','65','66','67','69','70','79','83','84','85','86','89','90','91','93','94','95','96','97')) THEN 'Tarjeta Credito'
              ...
              FROM
                      (SELECT ITM_TXN_AMT,TAX_AMOUNT,TXN_KEY,CUSTOMER_KEY,CUSTOMER_HEX,NBR_PD_ITM,WGHT_ITM,STORE_KEY,PRODUCT_KEY_1,DATE_KEY,    MARKET_BASKET_KEY,ITM_TXN_FCN_TP_DSC,FNC_DOC_TP_HEX FROM `cl-cda-prod.DS_CDA_VW_SMU.DW_VW_FACT_ITM_TXN`
                      WHERE DATE_TRUNC(ITM_TXN_TMS, MONTH) = PARSE_DATETIME('%Y%m', CAST(ANOMES_CERRADO AS STRING))
                      --DATE_TRUNC(ITM_TXN_TMS, MONTH) = DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), MONTH)
                      )   FIT -- FILTRO DE FECHA
                    ...
                    LEFT JOIN
                                (
                                    SELECT DISTINCT CUSTOMER_KEY, CLASIFICACION_CLIENTE
                                  FROM `cl-bigdata-analytics-preprod.ECOMMERCE.ECOMMERCE_CUSTOMER_SEGMENTATION_SOPHISTICATION` SO
                                  WHERE CAST(SO.MONTHID AS INT64) = ANOMES_CERRADO
                                  and STORE_BANNER = 'Unimarc'
                                  ) SPH
              ON
              ...
              SELECT
                ...
                , S.ORG_IP_ID
              FROM
                `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_FACT_WEEK_CUSTOMER_ORGANIZATION_SHABITS_ECOMMERCE` S
              JOIN
                (
                  SELECT
                    DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER AS MONTH_ID
                    , MAX(DY.CALENDAR_YEAR * 100 + DY.WEEK_NUMBER) AS MAX_SEMANA
                  FROM
                    `cl-cda-prod.DS_CDA_VW_SMU.DW_VW_DIM_DATE` DD
                  JOIN `cl-cda-prod.DS_CDA_VW_SMU.DW_VW_DIM_CALENDAR_YEAR_WEEK` DY ON DY.CALENDAR_YEAR_WEEK_KEY = DD.CALENDAR_YEAR_WEEK_KEY
                  WHERE
                    DY.CALENDAR_YEAR >= 2024
                  GROUP BY
                    1
                ) MAX_SEMANA
                ON
                  MAX_SEMANA.MAX_SEMANA = S.WEEK_ID
            ) SHA
                ON SHA.CUSTOMER_KEY = FIT.CUSTOMER_KEY
                AND SHA.MONTH_ID = DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER
              ...
              ON
                ...
                AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = CAST(FORMAT_DATE('%Y%m',DATE_ADD(PARSE_DATE('%Y%m',CAST(O.MONTH_ID AS STRING)), INTERVAL -1 MONTH)) AS INT64)
                    ...
                    -- AND FIT.CUSTOMER_HEX NOT IN ('3588d47a76aac91fcf2a3c2f55f6a351')
                      ...
                      AND DSH.STORE_ID NOT IN ('0622')
                      AND DSH.ORG_IP_ID IN ('01')
                      AND FBE.MARKET_BASKET_KEY IS NOT NULL
                      --AND COL.CUSTOMER_KEY IS NULL
                      AND O.CUSTOMER_KEY IS NULL
                      AND SPH.CUSTOMER_KEY IS NOT NULL
                    -- AND EQX.fecha_nacimiento IS NOT NULL
    ...
    *
    ...
    -- SUBQUERY --
        ...
        ,'69','70','79','83','84','85','86','89','90','91','93','94','95','96','97') OR B.TNDR_TP_ID IN ('42','44','45','46','60','61','62','64','65','66','67','69','70','79','83','84','85','86','89','90','91','93','94','95','96','97')) THEN 'Tarjeta Credito'
                        ...
                        ELSE '65 O MÁS'
                        END AS RANGO_EDAD,
                  CASE WHEN SHA.SHABITS = 'VIP Platino' THEN 'VIP'
                  WHEN SHA.SHABITS IS NULL THEN 'BRONCE'
                  WHEN SHA.SHABITS = 'EN FUGA' THEN 'BRONCE'
                  ELSE SHA.SHABITS END AS SHABITS,
                  SPH.CLASIFICACION_CLIENTE,
                  SUM(FIT.ITM_TXN_AMT - FIT.TAX_AMOUNT) AS VENTA_NETA,
                  COUNT(DISTINCT CASE WHEN FIT.ITM_TXN_AMT > 0 THEN FIT.CUSTOMER_KEY END) AS CLIENTES
              ...
              INNER JOIN `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_CDA_CST_DEID` DEID ON DEID.CUSTOMER_KEY  = FIT.CUSTOMER_KEY
                  ...
                  LEFT JOIN
                            (
                                  SELECT DISTINCT CUSTOMER_KEY, CLASIFICACION_CLIENTE
                                  FROM `cl-bigdata-analytics-preprod.CONOCIMIENTO_CLIENTE.CUSTOMER_SEGMENTATION_SOPHISTICATION`
                                  WHERE CAST(MONTHID AS INT64) = ANOMES_CERRADO
                                  and STORE_BANNER IN ('Mayorista','Super 10')
                                  ) SPH
              ON
              SPH.CUSTOMER_KEY =  FIT.CUSTOMER_KEY
                  ...
                  WHERE ORGANIZATION_ID IN (4,15) AND WEEK_ISO_ID=(SELECT MAX(WEEK_ISO_ID) FROM `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_FACT_WEEK_CUSTOMER_ORGANIZATION_SHABITS`)
                  AND SHABITS IS NOT NULL
                  )
          ...
          ON
            SPH.CUSTOMER_KEY =  FIT.CUSTOMER_KEY
                ...
                (
                  SELECT *
                  FROM
                  `cl-cda-unidata-prod.DS_PROD_CLIENTES_IC.VW_FACT_MONTH_CUSTOMER_ORGANIZATION_SHABITS_UNIDATA_ALVI`
                  WHERE
                  MONTHID = ANOMES_CERRADO
                  AND ORG_IP_ID IN ('08')
                )
          ...
          ON
            ...
            AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = CAST(FORMAT_DATE('%Y%m',DATE_ADD(PARSE_DATE('%Y%m',CAST(O.MONTH_ID AS STRING)), INTERVAL -1 MONTH)) AS INT64)
                ...
                -- AND FIT.CUSTOMER_HEX NOT IN ('3588d47a76aac91fcf2a3c2f55f6a351')
                  ...
                  AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = ANOMES_CERRADO
                  AND DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) >= 25
                  AND  NEW_MARCA.CUSTOMER_TYPE_DET NOT IN ('COMERCIANTE_DECLARADO','COMERCIANTE_INFERIDO')
                GROUP BY
                  1,2,3,4,5,6,7,8
                ORDER BY 1 ASC
    ), CLIENTES_TH_NO_USA AS (
            ...
            ON
              ...
              AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = CAST(FORMAT_DATE('%Y%m',DATE_ADD(PARSE_DATE('%Y%m',CAST(O.MONTH_ID AS STRING)), INTERVAL -1 MONTH)) AS INT64)
                  ...
                  -- AND FIT.CUSTOMER_HEX NOT IN ('3588d47a76aac91fcf2a3c2f55f6a351')
                    ...
                    AND DD.CALENDAR_YEAR * 100 + DD.CALENDAR_MONTH_NUMBER = ANOMES_CERRADO
                    AND DATE_DIFF(CURRENT_DATE(), CAST(EQX.fecha_nacimiento AS DATE), YEAR) >= 25
                    AND  NEW_MARCA.CUSTOMER_TYPE_DET IN ('COMERCIANTE_DECLARADO','COMERCIANTE_INFERIDO')
                  GROUP BY
                    1,2,3,4,5,6,7,8
                  ORDER BY 1 ASC
      ), CLIENTES_TH_NO_USA AS (
